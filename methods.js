const users = [
  ...
  function getAllNames(array) {
    console.log(array.map(el => el.username))
  }
  ...
  function removeAddresses(array) {
    return array.filter(el => delete el.addresses)
  }
  ...
  function getUsersGeo(array, id) {
    return array.find(function(items) {
        if (items.id == id) {
          console.log(items.address.geo)
        }
      })
